...
class shallowCNN(nn.Module):
    def __init__(
        ...
        self.norm0 = nn.BatchNorm2d(output_channel//2)
        self.max_pool = nn.MaxPool2d(
...
class LocalityAwareFeedforward(nn.Module):
    ...
    def forward(self, input, shape):
        input = input.transpose(-1,-2).reshape(*shape).contiguous()
        b, c, h, w = shape
        input = self.layers(input)
        input = input.reshape(b, c, h*w).transpose(-1, -2) # shape (batch_size, H*W, C).
        return input
...
class PositionalEncoding2D(nn.Module):
    ...
    def forward(self, input):
        ...
        out = input + h_pos_encoding + w_pos_encoding
