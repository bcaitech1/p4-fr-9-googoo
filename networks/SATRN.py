...
class BottleneckBlock(nn.Module):
    ...
    def __init__(self, input_size, growth_rate, dropout_rate=0.2, num_bn=3):
        """add more features(channels) as much as growth_rate. no change in width and height.
        Args:
    ...
    def forward(self, x):
        '''
        Args:
            x (Tensor): shape (batch_size, C, H, W)
        '''
...
class TransitionBlock(nn.Module):
    ...
    def forward(self, x):
        out = self.conv(self.relu(self.norm(x))) ## shape (batch_size, output_size, H, W)
        return self.pool(out) ## shape (batch_size, output_size, H/2, W/2)
...
class DenseBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        Args:
            x (Tensor): shape (batch_size, C, H, W).
        Returns:
            (Tensor): shape (batch_size, C + growth_rate*depth, H, W).
        '''
        return self.block(x)
    ...
    ***** this might be a point to be improved !!
    ...
    def __init__(
        ...
        self.conv0 = nn.Conv2d(
            input_channel,  # 3 for rgb, 1 for grey
            num_in_features,  # 48
            kernel_size=7,
            stride=2,
            padding=3,
    ...
    def forward(self, input):
        ...
        '''
        out = self.conv0(input)  # (H, V, ) (batch_size, 48, 64, 64)
        out = self.relu(self.norm0(out))
        out = self.max_pool(out) # (batch_size, 48, 32, 32)
        out = self.block1(out) # (batch_size, 432, 32, 32)
        out = self.trans1(out) # (batch_size, 216, 16, 16)
        out = self.block2(out) # (batch_size, 600, 16, 16)
        out_before_trans2 = self.trans2_relu(self.trans2_norm(out)) # (batch_size, 600, 16, 16)
        out_A = self.trans2_conv(out_before_trans2) # (batch_size, 300, 16, 16)
...
class PositionalEncoding2D(nn.Module):
    def __init__(self, in_channels, max_h=64, max_w=128, dropout=0.1):
        ...
        self.h_position_encoder = self.generate_encoder(in_channels // 2, max_h) ## shape (64, 150)
        self.w_position_encoder = self.generate_encoder(in_channels // 2, max_w) ## shape (128, 150)
    ...
    def forward(self, input):
        ...
        '''self.h_position_encoder[:h, :] -> shape (16, 150)
        h_pos_encoding -> shape (16, 1, 150)
        '''
        h_pos_encoding = self.h_linear(h_pos_encoding)  # [H, 1, D] -> shape (16, 1, 150)

        w_pos_encoding = (
            self.w_position_encoder[:w, :].unsqueeze(0).to(input.get_device())
        )
        '''self.w_position_encoder[:w, :] -> shape (16, 150)
        w_pos_encoding -> shape (16, 1, 150)
        '''
...
class SATRN(nn.Module):
    ...
    def forward(self, input, expected, is_train, teacher_forcing_ratio):
        '''
        Args:
            input (Tensor): image Tensor. shape (batch_size, C, H, W)
            expected (Tensor): target token labels. shape (batch_size, trg_len). trg_len is maximum length of label in given batch.
            is_train (Bool)
