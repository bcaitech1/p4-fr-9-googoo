...
####                                                              이미지 경로 설정 : half_images2
def split_gt(groundtruth, proportion=1.0, test_percent=None):
    root = os.path.join(os.path.dirname(groundtruth), "half_images2")
...
class LoadDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        # torchvision 대신 albumentation 사용하기
        # reference : https://albumentations.ai/docs/examples/migrating_from_torchvision_to_albumentations/

        # PIL + albumentation 을 사용하는 경우
        # if self.transform:
        ...
        # 
        # PIL + torchvision 을 사용하는 경우
        # if self.transform:
        #     image = self.transform(image)
        
        # CV2 + albumentation 을 사용하는 경우
        if self.transform:
...
def dataset_loader(options, transformed):
    ...
    train_dataset = LoadDataset(
        train_data, options.data.token_paths, crop=options.data.crop, transform=transformed["train"], rgb=options.data.rgb
    )
    ...
    valid_dataset = LoadDataset(
        valid_data, options.data.token_paths, crop=options.data.crop, transform=transformed["validation"], rgb=options.data.rgb
    )
