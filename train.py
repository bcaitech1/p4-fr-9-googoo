        ...
        if do_eval: # <SOS>, <EOS>, <PAD>은 무시한다.
            for token in example:
                ...
                if token not in special_ids:
                    ...
                    break
        else: # special token까지 전부 string으로 바꾼다.
            for token in example:
                token = token.item()
                if token != -1:
                    string += data_loader.dataset.id_to_token[token] + " "
    ...
    with tqdm(
        ...
        for d in data_loader:
            ...
            with torch.set_grad_enabled(train):
                output = model(input, expected, train, teacher_forcing_ratio) ## shape (batch_size, seq_len - 1, # vocab).
